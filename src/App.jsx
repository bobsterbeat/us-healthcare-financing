...
function WorkforceEconomics() {
  ...
  return (
    <>
      ...
      <SH>The Inevitable Expansion of CRNA Practice</SH>
      <Sub>Why the GME cap, economics, and demographics all point in one direction</Sub>
      <TB>
        <P>Regardless of the political battles — and they are fierce, particularly in California — the structural forces driving CRNA expansion are largely irreversible. Understanding why requires looking at three converging pressures that no amount of lobbying can fully counteract:</P>
      </TB>

      <FeatureCards cards={[
        { icon: "🔒", title: "The GME Cap Is a Ceiling", titleColor: "#f87171", body: "Medicare froze anesthesiology residency slots at 1996 levels. The 1,000 new GME slots from the 2021 CAA are spread across all specialties and prioritise primary care and psychiatry — anesthesiology gets a handful at most. Meanwhile, surgical volume is growing (aging population, expanding ambulatory surgery indications, new procedures). The physician anesthesiologist supply cannot expand fast enough to meet demand. The projected shortfall by 2033: ~12,500 anesthesiologists, nearly 30% of the current workforce. CRNAs fill the gap by default." },
        { icon: "💰", title: "The Economics Are Unanswerable", titleColor: "#fbbf24", body: "An anesthesiologist costs a hospital $550–620k/year in total compensation. A CRNA costs $246–295k. Revenue per case is identical under fee-for-service. In a care team model (1:4 medical direction), one anesthesiologist + four CRNAs covers five rooms for ~$1.6M total cost. Five anesthesiologists covering the same rooms costs ~$3M. Hospital CFOs see this math clearly. As financial pressure on hospitals intensifies — Medicaid cuts, payer mix shifts, flat Medicare updates — the economic argument for CRNA-heavy models becomes harder to resist." },
        { icon: "🏥", title: "Rural Access Is the Policy Lever", titleColor: "#4ade80", body: "CRNAs are the sole anesthesia provider in >80% of rural US counties. In many rural hospitals, without CRNAs there is no surgical or obstetric service at all. This is the argument that drives state opt-out legislation: governors grant opt-out not because of ideology but because rural hospitals tell them they'll lose their OR if they can't use independent CRNAs. 25 states have opted out. The trajectory — particularly as rural hospital closures accelerate — points toward more, not fewer." },
        { icon: "📈", title: "No Cap on SRNA Training", titleColor: "#60a5fa", body: "There are ~140 accredited SRNA programs producing ~3,000+ graduates per year, with no federal cap on expansion. Programs are growing: DNP is now the entry degree (replacing MSN), increasing training depth. New hospital-based programs are launching specifically to address local workforce needs. The pipeline is expanding while the physician anesthesiology pipeline is structurally constrained. Over a 10–20 year horizon, the CRNA-to-anesthesiologist ratio will continue to shift." },
      ]} />

      <SH>The California Battleground</SH>
      <Sub>CSA vs CANA: the most intense scope-of-practice fight in the country</Sub>
      <TB>
        <P>California is one of the most contested states for anesthesia scope of practice — and one of the few large states that has NOT opted out of the federal physician supervision requirement for CRNAs. The fight pits the California Society of Anesthesiologists (CSA) against the California Association of Nurse Anesthetists (CANA), and it has been running for decades with enormous lobbying expenditure on both sides.</P>
        <P>The CSA position: physician anesthesiologists provide a higher level of training (4 years of residency after medical school vs 3 years of SRNA training after nursing degree), handle the most complex cases (cardiac, pediatric, trauma, obstetric emergencies), and the medical direction model provides a safety net that independent CRNA practice removes. They argue that California's diverse, high-acuity patient population requires physician-led anesthesia care and that opt-out would compromise patient safety.</P>
        <P>The CANA position: the safety data shows no difference in outcomes between CRNA-only and physician-led anesthesia across multiple large studies. The supervision requirement is an economic barrier, not a safety one — it restricts CRNA practice to protect physician income. Opt-out would improve access, particularly in underserved areas of the Central Valley and rural Northern California where anesthesiologist recruitment is extremely difficult. They point to the 25 states that have opted out with no measurable increase in complications.</P>
        <P>The reality, viewed dispassionately: both sides have legitimate arguments, but they're arguing about different things. The CSA is arguing about the ceiling (who handles the sickest patients in the most complex settings). CANA is arguing about the floor (who provides anesthesia access in settings where anesthesiologists aren't available or aren't economically viable). The care team model — physician direction of CRNAs — is a reasonable middle ground for academic and high-acuity settings. But mandating that model in every setting, including rural critical access hospitals and ambulatory surgery centers doing low-risk cases, creates access barriers that the supervision requirement was never designed to address.</P>
      </TB>

      <Callout><B>Where it's heading:</B> California has resisted opt-out so far, largely because the CSA is well-funded and politically connected in Sacramento. But the economic and demographic pressures are relentless. As ambulatory surgery expands, as same-day joint replacements become standard, as rural hospitals close, and as the anesthesiologist shortage deepens, the political calculus shifts. Several California legislators have introduced opt-out or modified-supervision bills in recent sessions. The question is not whether California's anesthesia practice model evolves — it's when, and in what form. The most likely trajectory is not full opt-out (politically too difficult) but a tiered model: independent CRNA practice authorised for defined low-risk settings (ASCs, office-based, rural) while maintaining physician direction for higher-acuity hospital cases. This mirrors what's happened in many states that technically haven't opted out but have relaxed supervision requirements through regulatory rather than legislative action.</Callout>

      <TB>
        <P>For the academic anesthesiologist, the strategic response is clear: lead the integration rather than resist it. Departments that build strong care team models, invest in SRNA education, develop quality metrics that demonstrate value, and position the physician anesthesiologist as the perioperative medicine specialist (not just the person in the OR) will thrive regardless of scope-of-practice outcomes. Departments that define their value solely as "being in the room" are defending a position that economics and demographics are eroding year by year.</P>
      </TB>
